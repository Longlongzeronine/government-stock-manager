...
import { searchItems, getItemByCode } from "@/lib/data.functions";
...
function Scanner() {
  ...
  async function lookup(code: string) {
    ...
    if (!value) return;
    try {
      const item = await getItemByCode({ data: { code: value } });
      if (!item) {
            ...
            <div className="space-y-3">
              ...
              {result && (
                <div className="rounded-lg border border-border bg-background p-3 text-sm">
                  ...
                  <Link to="/inventory" search={{ add: false }} className="mt-3 inline-flex text-sm font-medium text-primary">
                    Open inventory
                  </Link>
                </div>
              )}
