...
import { listItems, createItem, updateItem, listRisTransactions, createTransaction } from "@/lib/data.functions";
import { createIarForm, createIarItem, createRisForm, createRisItem, createIcsForm, createIcsItems, createParForm, createParItems } from "@/lib/forms.functions";
...
function FormsFlow() {
  ...
  const { data: transactions = [] } = useQuery({
    ...
    queryFn: async () => await listRisTransactions(),
  });
  ...
  async function saveIar() {
    ...
    let iarRecord;
    try {
      iarRecord = await createIarForm({ data: {
        iar_no: iar.iarNo,
      ...
      setSaving(false);
      return toast.error(e?.message ?? "Failed to create IAR");
    }

    for (const line of validLines) {
      let tx;
      try {
        tx = await createTransaction({ data: {
        ...
        setSaving(false);
        return toast.error(e?.message ?? "Transaction failed");
      }
      ...
      try {
        ...
        setSaving(false);
        return toast.error(e?.message ?? "Failed to save IAR item");
      }
    }
  ...
  async function saveRis() {
    ...
    try {
      ...
      setSaving(false);
      return toast.error(e?.message ?? "Failed to create RIS");
    }
    ...
    for (const line of validLines) {
      ...
      try {
        ...
        setSaving(false);
        return toast.error(e?.message ?? "Failed to save RIS item");
      }
...
async function createAccountabilityDocuments({
  ...
  if (semiExpendable.length > 0) {
    try {
      ...
      await createIcsItems({ data: {
        items: semiExpendable.map((line) => ({
          ics_id: ics.id,
          item_id: line.item_id,
          quantity: Number(line.quantity),
      ...
      return e?.message ?? "Failed to create ICS documents";
    }
  }
  ...
  if (ppe.length > 0) {
    try {
      ...
      return e?.message ?? "Failed to create PAR documents";
    }
  }
