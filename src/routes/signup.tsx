...
function SignupPage() {
  ...
  return (
    <div className="min-h-screen grid place-items-center bg-background p-6">
      <form onSubmit={onSubmit} className="w-full max-w-sm space-y-5 bg-card border border-border rounded-lg p-8">
        <div>
          <h1 className="text-2xl">Request access</h1>
          <p className="text-sm text-muted-foreground mt-1">New accounts are created with viewer access. An administrator may upgrade your role.</p>
        </div>
        <div className="space-y-3">
          ...
          <Field label="Password"><input type="password" className="input" minLength={6} required value={password} onChange={e=>setPassword(e.target.value)} /></Field>
        </div>
        ...
        <p className="text-sm text-muted-foreground text-center">
          Already enrolled? <Link to="/login" className="text-primary font-medium hover:underline">Sign in</Link>
        </p>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <label className="block"><span className="text-xs font-medium uppercase tracking-wider">{label}</span><div className="mt-1.5">{children}</div></label>;
}
