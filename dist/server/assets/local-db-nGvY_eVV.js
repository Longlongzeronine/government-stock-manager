...
function Connection(options, queues = {}, { onopen = noop$1, onend = noop$1, onclose = noop$1 } = {}) {
  ...
  let socket = null, cancelMessage, errorResponse = null, result = new Result(), incoming = Buffer.alloc(0), needsTypes = options.fetch_types, backendParameters = {}, statements = {}, statementId = Math.random().toString(36).slice(2), statementCount = 1, closedTime = 0, remaining = 0, hostIndex = 0, retries = 0, length = 0, delay = 0, rows = 0, serverSignature = null, nextWriteTimer = null, terminated = false, incomings = null, results = null, initial = null, ending = null, stream = null, chunk = null, ended = null, nonce = null, query2 = null, final = null;
  const connection2 = {
    queue: queues.closed,
    idleTimer,
    connect(query3) {
  ...
  function execute(q) {
    ...
    try {
      q.state = backend;
      query2 ? sent.push(q) : (query2 = q, query2.active = true);
      build(q);
  ...
  function data(x) {
    ...
    while (incoming.length > 4) {
      ...
      try {
        ...
        query2 && (query2.cursorFn || query2.describeFirst) && write(Sync);
        errored(e);
      }
  ...
  function errored(err) {
    ...
    query2 && queryError(query2, err);
    initial && (queryError(initial, err), initial = null);
  }
  function queryError(query3, err) {
    if (query3.reserve)
      return query3.reject(err);
    if (!err || typeof err !== "object")
      err = new Error(err);
    "query" in err || "parameters" in err || Object.defineProperties(err, {
      stack: { value: err.stack + query3.origin.replace(/.*\n/, "\n"), enumerable: options.debug },
      query: { value: query3.string, enumerable: options.debug },
      parameters: { value: query3.parameters, enumerable: options.debug },
      args: { value: query3.args, enumerable: options.debug },
      types: { value: query3.statement && query3.statement.types, enumerable: options.debug }
    });
    query3.reject(err);
  }
  function end() {
    return ending || (!connection2.reserved && onend(connection2), !connection2.reserved && !initial && !query2 && sent.length === 0 ? (terminate(), new Promise((r) => socket && socket.readyState !== "closed" ? socket.once("close", r) : r())) : ending = new Promise((r) => ended = r));
  }
  function terminate() {
    terminated = true;
    if (stream || query2 || initial || sent.length)
  ...
  async function closed(hadError) {
    ...
    if (initial)
      return reconnect();
    !hadError && (query2 || sent.length) && error(Errors.connection("CONNECTION_CLOSED", options, socket));
  ...
  function DataRow(x) {
    ...
    let column;
    let value;
    const row = query2.isRaw ? new Array(query2.statement.columns.length) : {};
    for (let i = 0; i < query2.statement.columns.length; i++) {
      column = query2.statement.columns[i];
      length2 = x.readInt32BE(index);
      index += 4;
      value = length2 === -1 ? null : query2.isRaw === true ? x.subarray(index, index += length2) : column.parser === void 0 ? x.toString("utf8", index, index += length2) : column.parser.array === true ? column.parser(x.toString("utf8", index + 1, index += length2)) : column.parser(x.toString("utf8", index, index += length2));
      query2.isRaw ? row[i] = query2.isRaw === true ? value : transform.value.from ? transform.value.from(value, column) : value : row[column.name] = transform.value.from ? transform.value.from(value, column) : value;
    }
    query2.forEachFn ? query2.forEachFn(transform.row.from ? transform.row.from(row) : row, result) : result[rows++] = transform.row.from ? transform.row.from(row) : row;
  }
  ...
  function ReadyForQuery(x) {
    if (query2) {
      if (errorResponse) {
        query2.retried ? errored(query2.retried) : query2.prepared && retryRoutines.has(errorResponse.routine) ? retry(query2, errorResponse) : errored(errorResponse);
      } else {
        query2.resolve(results || result);
      }
    } else if (errorResponse) {
      errored(errorResponse);
    }
    query2 = results = errorResponse = null;
    result = new Result();
    connectTimer.cancel();
    if (initial) {
    ...
    while (sent.length && (query2 = sent.shift()) && (query2.active = true, query2.cancelled))
      Connection(options).cancel(query2.state, query2.cancelled.resolve, query2.cancelled.reject);
    if (query2)
      return;
  ...
  function CommandComplete(x) {
    ...
    if (query2.options.simple)
      return BindComplete();
    if (query2.cursorFn) {
      result.count && query2.cursorFn(result);
      write(Sync);
    }
  }
  function ParseComplete() {
    query2.parsing = false;
  }
  function BindComplete() {
    !result.statement && (result.statement = query2.statement);
    result.columns = query2.statement.columns;
  }
  function ParameterDescription(x) {
    const length2 = x.readUInt16BE(5);
    for (let i = 0; i < length2; ++i)
      !query2.statement.types[i] && (query2.statement.types[i] = x.readUInt32BE(7 + i * 4));
    query2.prepare && (statements[query2.signature] = query2.statement);
    query2.describeFirst && !query2.onlyDescribe && (write(prepared(query2)), query2.describeFirst = false);
  }
  function RowDescription(x) {
    if (result.command) {
      ...
      result.count = null;
      query2.statement.columns = null;
    }
    const length2 = x.readUInt16BE(5);
    let index = 7;
    let start;
    query2.statement.columns = Array(length2);
    for (let i = 0; i < length2; ++i) {
      start = index;
      while (x[index++] !== 0) ;
    ...
    result.statement = query2.statement;
    if (query2.onlyDescribe)
      return query2.resolve(query2.statement), write(Sync);
  }
  ...
  function ErrorResponse(x) {
    if (query2) {
      (query2.cursorFn || query2.describeFirst) && write(Sync);
  ...
  async function PortalSuspended() {
    try {
      const x = await Promise.resolve(query2.cursorFn(result));
      rows = 0;
      x === CLOSE ? write(Close(query2.portal)) : (result = new Result(), write(Execute("", query2.cursorRows)));
    } catch (err) {
      write(Sync);
      query2.reject(err);
    }
...
function Postgres(a, b2) {
  ...
  function Sql(handler2) {
    ...
    function sql3(strings, ...args) {
      const query2 = strings && Array.isArray(strings.raw) ? new Query(strings, args, handler2, cancel) : typeof strings === "string" && !args.length ? new Identifier(options.transform.column.to ? options.transform.column.to(strings) : strings) : new Builder(strings, args);
      return query2;
    }
    function unsafe(string, args = [], options2 = {}) {
      arguments.length === 2 && !Array.isArray(args) && (options2 = args, args = []);
      const query2 = new Query([string], args, handler2, cancel, {
        prepare: false,
        ...options2,
    ...
    function file(path, args = [], options2 = {}) {
      arguments.length === 2 && !Array.isArray(args) && (options2 = args, args = []);
      const query2 = new Query([], args, (query3) => {
        fs.readFile(path, "utf8", (err, string) => {
          if (err)
            return query3.reject(err);
          query3.strings = [string];
          handler2(query3);
        });
      }, cancel, {
        ...options2,
  ...
  async function reserve() {
    ...
    const c = open.length ? open.shift() : await new Promise((resolve, reject) => {
      const query2 = { reserve: resolve, reject };
      queries.push(query2);
      closed.length && connect(closed.shift(), query2);
    });
    move(c, reserved);
  ...
  function handler(query2) {
    if (ending)
      return query2.reject(Errors.connection("CONNECTION_ENDED", options, options));
    if (open.length)
      return go(open.shift(), query2);
    if (closed.length)
      return connect(closed.shift(), query2);
    busy.length ? go(busy.shift(), query2) : queries.push(query2);
  }
  function go(c, query2) {
    return c.execute(query2) ? move(c, busy) : move(c, full);
  }
  function cancel(query2) {
    return new Promise((resolve, reject) => {
      query2.state ? query2.active ? Connection(options).cancel(query2.state, resolve, reject) : query2.cancelled = { resolve, reject } : (queries.remove(query2), query2.cancelled = true, query2.reject(Errors.generic("57014", "canceling statement due to user request")), resolve());
    });
  }
  ...
  function onopen(c) {
    ...
    while (ready && queries.length && max-- > 0) {
      const query2 = queries.shift();
      if (query2.reserve)
        return query2.reserve(c);
      ready = c.execute(query2);
    }
...
function parseOptions(a, b2) {
  if (a && a.shared)
    return a;
  const env = process$1.env, o = (!a || typeof a === "string" ? b2 : a) || {}, { url, multihost } = parseUrl(a), query2 = [...url.searchParams].reduce((a2, [b3, c]) => (a2[b3] = c, a2), {}), host = o.hostname || o.host || multihost || url.hostname || env.PGHOST || "localhost", port = o.port || url.port || env.PGPORT || 5432, user = o.user || o.username || url.username || env.PGUSERNAME || env.PGUSER || osUsername();
  o.no_prepare && (o.prepare = false);
  query2.sslmode && (query2.ssl = query2.sslmode, delete query2.sslmode);
  "timeout" in o && (console.log("The timeout option is deprecated, use idle_timeout instead"), o.idle_timeout = o.timeout);
  query2.sslrootcert === "system" && (query2.ssl = "verify-full");
  ...
  return {
    ...
    ...Object.entries(defaults).reduce(
      (acc, [k, d]) => {
        const value = k in o ? o[k] : k in query2 ? query2[k] === "disable" || query2[k] === "false" ? false : query2[k] : env["PG" + k.toUpperCase()] || d;
    ...
    connection: {
      ...
      ...o.connection,
      ...Object.entries(query2).reduce((acc, [k, v]) => (k in defaults || (acc[k] = v), acc), {})
    },
    types: o.types || {},
...
let sql = null;
function getDbUrl() {
  return process.env.DATABASE_URL || process.env.VITE_DATABASE_URL || "postgres://postgres:postgres@localhost:5432/government_stock_manager";
}
function getSql() {
  if (!sql) {
...
async function query(strings, ...values2) {
  const s = getSql();
  return s(strings, ...values2);
}
async function queryRaw(sqlStr, ...params) {
  const s = getSql();
